      ...
      <div class="column column-center">
        <div class="panel panel-map" :style="{ '--zoom-level': mapZoomLevel }">
          ...
          <div class="map-wrapper">
            ...
            <div 
              v-if="hoveredProvince && !showProvincePanel" 
              class="province-card" 
              :class="{ 'no-detail': !hoveredProvince.hasDetailData }"
              :style="provinceCardStyle"
              @mouseenter="cancelHideProvinceCard"
              @mouseleave="hideProvinceCard"
              @click="hoveredProvince.hasDetailData && openProvincePanel(hoveredProvince.name)"
            >
              ...
              <div class="card-hint" v-if="hoveredProvince.hasDetailData">点击查看详情</div>
            </div>
          </div>
        </div>
        
        <ProvincePanel 
          v-model:visible="showProvincePanel" 
          :province-name="selectedProvince"
          @close="showProvincePanel = false"
        />
...
<script setup>
import { ref, onMounted, onUnmounted, shallowRef, computed, nextTick, h, watch } from 'vue'
...
import ScrollNotice from '@/components/business/ScrollNotice.vue'
import ProvincePanel from '@/components/business/ProvincePanel.vue'
...
const showProvincePanel = ref(false)
const selectedProvince = ref('')

let hideProvinceCardTimeout = null

const openProvincePanel = (provinceName) => {
  selectedProvince.value = provinceName
  showProvincePanel.value = true
}

const cancelHideProvinceCard = () => {
  if (hideProvinceCardTimeout) {
    clearTimeout(hideProvinceCardTimeout)
    hideProvinceCardTimeout = null
  }
}
...
const showProvinceCard = (province, event) => {
  ...
  const hasDetailData = !!provinceCultureData.provinces[province.name]
...
const hideProvinceCard = () => {
  hideProvinceCardTimeout = setTimeout(() => {
