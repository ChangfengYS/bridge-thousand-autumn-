...
const trendKeywords = ['拱桥', '梁桥', '廊桥', '索桥', '石桥', '木桥', '石拱', '铁索', '廊屋', '园林']
...
const updateCompareChart = () => {
  ...
  const data1 = getWordCloudData(compareDynasty1.value).slice(0, 10)
  const data2 = getWordCloudData(compareDynasty2.value).slice(0, 10)
  const allKeywords = new Set([...data1.map(d => d.name), ...data2.map(d => d.name)])
  ...
  compareChart.value.setOption({
    ...
    series: [
      { 
        ...
        type: 'bar', 
        data: keywords.map(k => data1.find(d => d.name === k)?.value || 0), 
        barWidth: '30%',
        barGap: '15%',
        itemStyle: {
      ...
      { 
        ...
        type: 'bar', 
        data: keywords.map(k => data2.find(d => d.name === k)?.value || 0), 
        barWidth: '30%',
        itemStyle: {
          borderRadius: [4, 4, 0, 0],
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: '#E07040' },
            { offset: 0.5, color: '#C94A3A' },
            { offset: 1, color: '#A53035' }
          ]),
          shadowColor: 'rgba(224, 112, 64, 0.3)',
          shadowBlur: 8,
          shadowOffsetY: 2
        },
        emphasis: {
...
const initWordTrendChart = () => {
  ...
  wordTrendChart.value.setOption({
    ...
    xAxis: {
      type: 'category',
      data: dynastyList,
      axisLabel: { color: '#E8E4D8', fontSize: 9, rotate: 30 },
    ...
    series: [{
      ...
      areaStyle: {
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          { offset: 0, color: 'rgba(201, 169, 97, 0.4)' },
          { offset: 1, color: 'rgba(201, 169, 97, 0.05)' }
        ])
      },
...
const updateTrendChart = () => {
  ...
  const colors = [
    { line: '#C9A961', area: 'rgba(201, 169, 97, 0.3)' },
    { line: '#B8423A', area: 'rgba(184, 66, 58, 0.3)' },
    { line: '#5B8C85', area: 'rgba(91, 140, 133, 0.3)' },
    { line: '#6B8E23', area: 'rgba(107, 142, 35, 0.3)' },
    { line: '#CD853F', area: 'rgba(205, 133, 63, 0.3)' },
    { line: '#D66B5A', area: 'rgba(214, 107, 90, 0.3)' },
    { line: '#8B7355', area: 'rgba(139, 115, 85, 0.3)' },
    { line: '#BC8F8F', area: 'rgba(188, 143, 143, 0.3)' },
    { line: '#F4A460', area: 'rgba(244, 164, 96, 0.3)' },
    { line: '#DAA520', area: 'rgba(218, 165, 32, 0.3)' }
  ]
  ...
  trendChart.value.setOption({
    ...
    xAxis: {
      ...
      axisTick: { 
        show: true,
        lineStyle: { color: 'rgba(201, 169, 97, 0.5)' }
      },
      splitLine: {
        show: true,
        lineStyle: {
    ...
    yAxis: {
      type: 'value',
      axisLabel: { color: 'rgba(232, 228, 216, 0.7)', fontSize: 11 },
      splitLine: { lineStyle: { color: 'rgba(201, 169, 97, 0.12)', type: 'dashed' } },
      axisLine: { show: false },
      axisTick: { show: false }
    },
...
const updateCrossChart = () => {
  ...
  const dim1Values = crossDim1.value === 'dynasty' ? dynastyList : typeList.map(t => t.label)
  const dim2Values = crossDim2.value === 'dynasty' ? dynastyList : typeList.map(t => t.label)
  
  const data = []
  dim1Values.forEach((d1, i) => {
    dim2Values.forEach((d2, j) => {
      data.push([i, j, Math.floor(Math.random() * 100) + 20])
    })
  })
  ...
  crossChart.value.setOption({
    tooltip: {
      ...
      padding: [12, 16],
      textStyle: { color: '#E8E4D8', fontSize: 13 },
      formatter: (params) => `${dim1Values[params.data[0]]} × ${dim2Values[params.data[1]]}<br/>词频: ${params.data[2]}`
    },
    grid: { left: '15%', right: '10%', bottom: '15%', top: '10%' },
    xAxis: {
      type: 'category',
      data: dim2Values,
      axisLabel: { color: '#E8E4D8', fontSize: 10, rotate: 30 },
      axisLine: { lineStyle: { color: 'rgba(201, 169, 97, 0.5)' } },
      splitArea: { show: true, areaStyle: { color: ['rgba(201, 169, 97, 0.05)', 'rgba(201, 169, 97, 0.1)'] } }
    },
    yAxis: {
      type: 'category',
      data: dim1Values,
    ...
    visualMap: {
      ...
      bottom: '0%',
      inRange: {
        color: ['#1A3A3A', '#4A6B6B', '#5B8C85', '#8B7355', '#C9A961', '#FFD700']
      },
      textStyle: { color: '#E8E4D8' }
    },
    series: [{
      ...
      label: { show: false },
      emphasis: {
        itemStyle: { shadowBlur: 10, shadowColor: 'rgba(201, 169, 97, 0.5)' }
      }
    }]
  })
}
...
.toolbar {
  ...
  .select {
    ...
    &:hover {
      ...
      background-color: rgba(45, 90, 90, 0.45);
    }
...
.relation-header {
  ...
  background: linear-gradient(135deg, rgba(201, 169, 97, 0.08) 0%, rgba(212, 175, 55, 0.03) 100%);
  ...
  .close-btn {
    ...
    border: 1px solid rgba(184, 66, 58, 0.25);
    border-radius: 50%;
    color: #B8423A;
    font-size: 20px;
    cursor: pointer;
...
.word-related-section {
  ...
  .related-tag {
    ...
    &.bridge-tag {
      ...
      &:hover {
        background: linear-gradient(135deg, rgba(184, 66, 58, 0.3) 0%, rgba(184, 66, 58, 0.2) 100%);
        transform: translateY(-2px);
        box-shadow: 0 4px 12px rgba(184, 66, 58, 0.2);
      }
    }
...
.word-actions {
  ...
  .action-btn {
    ...
    &.search {
      background: linear-gradient(135deg, rgba(45, 90, 90, 0.4) 0%, rgba(45, 90, 90, 0.25) 100%);
      border-color: rgba(91, 140, 133, 0.4);
      color: #5B8C85;

      &:hover {
        background: linear-gradient(135deg, rgba(91, 140, 133, 0.3) 0%, rgba(91, 140, 133, 0.2) 100%);
        transform: translateY(-2px);
